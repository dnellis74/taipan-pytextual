...
class BattleScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            Static("", id="battle-status"),
            Static("", id="battle-message"),
            Static("", id="battle-ships"),
            Static("", id="battle-orders"),
            id="battle-container"
        )
    ...
    def on_mount(self) -> None:
        ...
        self._update_battle_message("Taipan, what shall we do??    (f=Fight, r=Run, t=Throw cargo)")
    ...
    def _update_battle_status(self) -> None:
        ...
        self.query_one("#battle-status", Static).update(
    ...
    def _update_battle_message(self, message: str) -> None:
        """Update the battle message display."""
        self.query_one("#battle-message", Static).update(message)
    ...
    def _handle_fight(self) -> None:
        ...
        if self.game_state.guns == 0:
            self._update_battle_message("We have no guns, Taipan!!")
            return
        
        self._update_battle_message("Aye, we'll fight 'em, Taipan.")
        time.sleep(3)
        
        self._update_battle_message("We're firing on 'em, Taipan!")
        time.sleep(1)
        ...
        for i in range(1, self.game_state.guns + 1):
            ...
            if i < self.game_state.guns:
                self._update_battle_message(f"({self.game_state.guns - i} shots remaining.)")
                time.sleep(0.5)
        
        if sk > 0:
            self._update_battle_message(f"Sunk {sk} of the buggers, Taipan!")
        else:
            self._update_battle_message("Hit 'em, but didn't sink 'em, Taipan!")
        time.sleep(3)
        ...
        if (random.randint(1, self.original_ships) > (self.num_ships * 0.6 / self.battle_type) and 
            ...
            self._update_battle_status()
            self._update_battle_message(f"{ran} ran away, Taipan!")
            time.sleep(3)
    ...
    def _handle_run(self) -> None:
        """Handle run orders."""
        self._update_battle_message("Aye, we'll run, Taipan.")
        time.sleep(3)
        ...
        if random.randint(1, self.ok) > random.randint(1, self.num_ships):
            self._update_battle_message("We got away from 'em, Taipan!")
            time.sleep(3)
            self.num_ships = 0
        else:
            self._update_battle_message("Couldn't lose 'em.")
            time.sleep(3)
            ...
            if self.num_ships > 2 and random.randint(1, 5) == 1:
                ...
                self._update_battle_status()
                self._update_battle_message(f"But we escaped from {lost} of 'em!")
                time.sleep(3)
    ...
    def _handle_throw_cargo(self) -> None:
        ...
        self._update_battle_message("What shall I throw overboard, Taipan? (o=Opium, s=Silk, a=Arms, g=General, *=All)")
        # TODO: Handle cargo selection and amount
        pass
    
    def _handle_enemy_attack(self) -> BattleResult:
        """Handle enemy attack."""
        self._update_battle_message("They're firing on us, Taipan!")
        time.sleep(3)
        
        # TODO: Implement visual attack effect
        
        self._update_battle_message("We've been hit, Taipan!!")
        time.sleep(3)
        ...
        if (self.game_state.guns > 0 and 
            ...
            self._update_battle_message("The buggers hit a gun, Taipan!!")
            time.sleep(3)
    ...
    def on_key(self, event: events.Key) -> None:
        ...
        if self.num_ships > 0:
            result = self._handle_enemy_attack()
        ...
        if self.num_ships == 0:
            if self.orders == 1:
                self._update_battle_message("We got 'em all, Taipan!")
                time.sleep(3)
                self.app.pop_screen()
                return
            else:
